...
mpg_model_class <- mpg %>% # start with mpg
  unite(col = model_class, # create a new column called 'model_class'
        ...
        sep = "-", # puts "-" between the pieces
        remove = FALSE) # do not remove original columns
...
```{r}
# Your code here: 
some_dates <- c(11061972, 02142007,01301843) # creates a vector of dates without spaces
some_dates_converted <- mdy(some_dates) # converts to dates, stores as some_dates_converted
some_dates_converted # returnes some_dates_converted
class(some_dates_converted) #checks the class
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>% # start with lakers
  filter(player == "Pau Gasol") %>% # filter to only include player Pau Gasol
  filter(type == "hook") %>% # filter to only include hook shots
  mutate(date = ymd(date)) %>% # convert date column to "Date" class with ymd format
  mutate(month = month(date)) # create new column from date that only contains month
  
count(gasol_hookshots, vars = "month") # count the number of variables that have a value for the month column
...
storms_danger <- storms %>% # start from storms data
  mutate(danger = case_when( # add a new column "danger" which contains...
    ...
    status != "hurricane" ~ "moderate" # "moderate if status is not "hurricane" (can also be done with TRUE ~ "moderate")
  ))
